...
import main_helpers
from main_helpers import ADMIN_STATES
from main_handlers import start_handler, cancel_handler, id_handler, explore_handler
...
def register_main_bot_handlers(app: Client):
    ...
    async def msg_state(client: Client, message: Message):
        ...
        from main_states_admin import handle_admin_states
        if await handle_admin_states(client, message, state, state_data, message_id):
            return

        from main_states_files import handle_files_states
    ...
    async def cb_query(client: Client, callback: CallbackQuery):
        ...
        from main_callbacks_config import handle_config_callbacks
        if await handle_config_callbacks(client, callback, data):
            return

        from main_callbacks_admin import handle_admin_callbacks
        if await handle_admin_callbacks(client, callback, data):
            return

        from main_callbacks_files import handle_files_callbacks
        if await handle_files_callbacks(client, callback, data):
            return

        from main_callbacks_series import handle_series_callbacks
